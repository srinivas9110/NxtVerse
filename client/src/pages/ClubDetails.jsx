...
                            // Organizer: Faculty, President, or Specifically Assigned
...
                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteWorkshop(workshop._id); }} className="absolute top-4 right-4 p-2 text-gray-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all z-20">
                                            <Trash2 size={18} />
                                        </button>
                                    )}
...
                                            {isRegistered ? (
                                                <button disabled className="px-6 py-2 bg-green-500/10 text-green-500 border border-green-500/20 rounded-lg font-bold flex items-center gap-2 cursor-default">
...
                                            {isOrganizer && (
                                                <button
                                                    onClick={() => { setDashboardModal(workshop); setSearchTerm(""); }}
                                                    className="px-4 py-2 bg-[#18181b] border border-white/10 hover:bg-white/5 text-white rounded-lg font-bold flex items-center gap-2 transition-all"
                                                >
...
                                        <input className="w-full bg-black/40 p-3 pl-10 rounded-lg text-white border border-white/5 text-sm focus:border-blue-500 transition-colors placeholder:text-gray-600" placeholder="Paste YouTube link..." value={newPost.link} onChange={e => setNewPost({ ...newPost, link: e.target.value })} disabled={uploading} />
                                    </div>
...
                                        {(isFaculty || isPresident) && (
                                            <button onClick={() => handleDeletePost(post._id)} className="ml-auto text-gray-500 hover:text-red-500 p-2"><Trash2 size={16} /></button>
                                        )}
                                    </div>
                                    <div className="p-4 text-gray-200 text-sm">{post.caption}</div>
                                    {isImage ? (
                                        <div className="w-full bg-black flex justify-center border-t border-white/5">
                                            <img src={post.link} alt="Post" className="max-h-[500px] w-auto object-contain" />
                                        </div>
...
                                    <span className="px-2 py-0.5 bg-purple-600 rounded text-xs font-bold text-white">Dashboard</span>
                                </div>
...
                                    className="w-full bg-[#121214] border border-white/10 p-3 pl-12 rounded-xl text-white outline-none focus:border-purple-500 transition-colors"
                                    value={searchTerm}
...
                            {dashboardModal.attendees
                                ?.filter(att => att.user?.fullName.toLowerCase().includes(searchTerm.toLowerCase()) || att.user?.collegeId.toLowerCase().includes(searchTerm.toLowerCase()))
                                .map(att => {
...
                                            <div>
                                                <h4 className="font-bold text-white text-base">{att.user.fullName}</h4>
                                                <p className="text-xs text-gray-500 font-mono mt-0.5 uppercase">
                                                    {att.user.collegeId} • {att.user.section} {att.user.role !== 'student' && `• ${att.user.role}`}
                                                </p>
                                            </div>

                                            <div className="flex items-center gap-3">
                                                {/* ORGANIZER TOGGLE (Only Faculty can assign) */}
                                                {isFaculty && (
                                                    <button
                                                        onClick={() => handleToggleOrganizer(dashboardModal._id, att.user._id)}
                                                        className={`p-2 rounded-lg transition-all ${isOrg ? 'text-purple-400 bg-purple-500/10' : 'text-gray-600 hover:text-gray-400'}`}
                                                        title={isOrg ? "Remove Organizer" : "Make Organizer"}
                                                    >
                                                        <Shield size={18} fill={isOrg ? "currentColor" : "none"} />
                                                    </button>
                                                )}

                                                {/* ATTENDANCE TOGGLE */}
                                                <button
                                                    onClick={() => handleMarkAttendance(dashboardModal._id, att.user._id)}
                                                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all border ${att.present
                                                        ? 'bg-green-500/10 text-green-400 border-green-500/20'
                                                        : 'bg-[#18181b] text-gray-400 border-white/10 hover:bg-white/5'
                                                        }`}
                                                >
                                                    {att.present ? "Present" : "Mark"}
                                                </button>
                                            </div>
                                        </div>
                                    );
...
                            {dashboardModal.attendees?.length === 0 && (
                                <div className="text-center py-10 text-gray-600">No students registered yet.</div>
                            )}
                        </div>
                    </div>
                </div>
